...
/*
 * yue: ssdb named it recv(). Its actual function is to parse the packet,
 * however. Hence, I changed the function name to parse() to avoid confusion.
 */
const std::vector<Bytes>* Link::parse(){
...
const std::vector<Bytes>* Link::response(){
	while(1){
		const std::vector<Bytes> *resp = this->parse();
		if(resp == NULL){
			return NULL;
		}else if(resp->empty()){
			if(this->read() <= 0){
